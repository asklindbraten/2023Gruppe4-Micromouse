...
//the x axis and the number of rows represent the total amount of squares on the y axis. These multiplied with eachother gives the total amount of nodes/cells in the entire maze.
    ...
    m_num_rows = num_rows;
    for (int i{0}; i < num_rows; i++){ //This double for-loop creates a 2D vector of nullptrs, so accessing different parts of the vector will not 
    //result in "vector subscript out of range".  
  ...
  Vector<Vector<std::shared_ptr<Node>>> Maze::getGrid(){
    return m_grid;
  }

  void Maze::setNewMaze(Vector<Vector<std::shared_ptr<Node>>> a){
    m_grid = a;
  } 
  //This function prints the x- and y index of a specific cell, aswell as the neighbouring nodes its potentially connected to. 
  void Maze::printPointerInfo(std::shared_ptr<Node> cell){
    Serial.print("(");
