...
//global variables for turn decisions, resetting and navigation:
...
void loop(){ 
  ...
  inititated = true;
  //Check if data is available to cleanup pointers.
  if (Serial.available() > 0){
      command = Serial.read();
      if (command == 'R'){
      Serial.print("Last active cell: (");
      micromouse->getActiveCell()->getXidx(); 
      Serial.print(", ");
      micromouse->getActiveCell()->getYidx();
      Serial.println(")");
      delete_raw_pointers();
      Serial.println("Raw pointers deleted");
      }
    }
...
void delete_raw_pointers(){
  for (int i{0}; i < micromouse->getMazeObj()->getNumRows(); i++){
    for (int j{0}; j < micromouse->getMazeObj()->getNumCol(); j++){
      if (micromouse->getMazeObj()->getGrid()[i][j] != nullptr){
        micromouse->getMazeObj()->getGrid()[i][j]->~Node();
        delete micromouse->getMazeObj()->getGrid()[i][j];
        micromouse->getMazeObj()->getGrid()[i][j] = nullptr;
      }
    }
  }
...
double adjMeasureToRWdist(double measurement){
  if (measurement < 1.84){
    return 0;
  } //Standard cases:
  else if (measurement >= 1.84 && measurement <= 8.28){
    return 1.33806 * measurement - 0.682216;
  }
  else if (measurement >= 10.17 && measurement <= 16.94){
    return 1.16526 * measurement + 0.203794;
  }
  else if (measurement >= 18.25 && measurement <= 24.95){
    return 1.25449 * measurement - 1.05304;
  }
  else if (measurement >= 26.31 && measurement <= 31.25){
...
void read_adj_set_sensor_val(){
  ...
  adj_dist_f = lowPass(filt_val_f, old_val_str, new_val_str, adjMeasureToRWdist(pre_dist_f));
  adj_dist_l = lowPass(filt_val_l, old_val_str, new_val_str, adjMeasureToRWdist(pre_dist_l));
  adj_dist_r = lowPass(filt_val_r, old_val_str, new_val_str, adjMeasureToRWdist(pre_dist_r));
...
void update_visited_dir(){
  if (micromouse->getPosition() == 0){
    if (adj_dist_f > limit && micromouse->getMazeObj()->getGrid()[act_x][act_y_inc]->getIsVisited() == false){
      visited_dir[0] = 0; //0 means open cell in the forward direction and is not visited. 
    } else if (adj_dist_f > limit && micromouse->getMazeObj()->getGrid()[act_x][act_y_inc]->getIsVisited() == true){
      visited_dir[0] = 1; //1 means open cell in the forward direction and is visited.
    } else if (adj_dist_f < limit){
      visited_dir[0] = 2; //2 means wall, so no cell in the forward direction. 
    }
   
    if (adj_dist_r > limit && micromouse->getMazeObj()->getGrid()[act_x_inc][act_y]->getIsVisited() == false){
      visited_dir[1] = 0; //same as above but for right sensor.
    ...
    if (adj_dist_l > limit && micromouse->getMazeObj()->getGrid()[act_x_dec][act_y]->getIsVisited() == false){
      visited_dir[2] = 0; //same as above but for left sensor.
...
void makeMovementDecision(){
   ...
   //We want the micromouse to make choices based on the visited_dir (visited_direction) vector. the value: 0 has priority over the value: 1 etc. 
