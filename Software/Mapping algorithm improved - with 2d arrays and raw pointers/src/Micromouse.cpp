...
Micromouse::Micromouse(double l_s_val, double r_s_val, double f_s_val, int position, Node* a, 
  Maze* b){ //Constructor with parameters to set membervariables.
  m_left_sensor_val = l_s_val;
  m_right_sensor_val = r_s_val; 
  m_front_sensor_val = f_s_val; 
  m_position = position;
  m_active_cell = a; 
  m_maze_obj = b;
  }
Micromouse::~Micromouse(){
  delete m_active_cell;
  delete m_maze_obj;
...
double Micromouse::getFrontSensorVal(){
  ...
  Node* Micromouse::getActiveCell(){
    return m_active_cell;
  }
  Maze* Micromouse::getMazeObj(){
    return m_maze_obj;
  }
  ...
  void Micromouse::setActiveCell(Node* n){
    m_active_cell = n;
  }
  void Micromouse::setMazeObj(Maze* n){
    m_maze_obj = n;
  }
  ...
  void Micromouse::connectNodesLeft(Node* current, Node* next, int pos){
    if (pos == 0){
  ...
  void Micromouse::connectNodesRight(Node* current, Node* next, int pos){
    if (pos == 0){
      current->setEastptr(next);
      next->setWestptr(current);
    }
  ...
  void Micromouse::connectNodesForw(Node* current, Node* next, int pos){
    if (pos == 0){
      current->setNorthptr(next);
      next->setSouthptr(current);
    }
 ...
 //The createAndConnect functions below creates a new neighbouring node and connects it to the active cell/node the micromouse is currently in, in the right, forward and left directions (seperately) based on position.
  void Micromouse::createAndConnectForwNode(int act_x, int act_y, int pos, Node*** grid){
      if (pos == 0){
        prev_y = act_y;
        new_y = act_y++;
        ...
        //the cell the micromouse is currently in.
          grid[act_x][new_y] = new Node(act_x, new_y, false);
      ...
      if (pos == 90 || pos == -270){
        ...
        if (grid[new_x][act_y] == nullptr){
          grid[new_x][act_y] = new Node(new_x, act_y, false);
    ...
    void Micromouse::createAndConnectRightNode(int act_x, int act_y, int pos, Node*** grid){
      if (pos == 0){
        prev_x = act_x;
        new_x = act_x++;
        if (grid[new_x][act_y] == nullptr){
      ...
      void Micromouse::createAndConnectLeftNode( int act_x, int act_y, int pos, Node* **grid){
      if (pos == 0){
        prev_x = act_x;
        new_x = act_x--;
        if (grid[new_x][act_y] == nullptr){
  ...
  //because the many if-tests of the createAndConnect functions were hardcoded into it.   
    ...
    Node*** grid_vector = m_maze_obj->getGrid(); //Making a local variable to store the temporary maze grid-vector.
...
void Micromouse::changeActiveCell(){ //This function changes the active cell for the micromouse based on forward movement in different positions.
  Node* prev_cell = m_active_cell; //temporary local variable to store the prev active cell. 
